...
<div>
   <el-breadcrumb separator-class="el-icon-arrow-right">
        <el-breadcrumb-item :to="{ path: '#' }">操作类型</el-breadcrumb-item>
    </el-breadcrumb>
    ...
    <el-row :gutter="20">
        <el-col :span="5">
         ...
         <el-col :span="5">
             <div class="grid-content">
                   <el-input placeholder="请输入操作类型" v-model="repair_type_name">
    ...
    <el-table :data="tableData" style="width: 100%" :row-class-name="tableRowClassName" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="repair_type_name" label="操作类型"></el-table-column>
    </el-table>
...
<!--分页-->
 ...
 <el-dialog title="操作类型" :visible.sync="dialogFormVisible" @close="handleClose">
     <el-form :model="form">
         <el-form-item label="操作类型" :label-width="formLabelWidth">
             <el-input style="width:240px" v-model="form.repair_type_name" auto-complete="off"></el-input>
         </el-form-item>
     </el-form>
...
export default {
    ...
    methods: {
      ...
      handleClose(){
        this.addForm = false;
        this.form.repair_type_name = "";
      },
      //确定Form
      handleConfirmAdd(){
        if(this.form.repair_type_name!=""){
            if(this.addForm){
                var url = UrlConfig.AddRepairType;
                var method ='post';
                var params =  {
                    repair_type_name:this.form.repair_type_name
                };
                request(url,method,params).then((res,err)=>{
                    ...
                    var url = UrlConfig.UpdateRepairType;
                    var method ='post';
                    var params =  {
                        repair_type:this.multipleSelection[0].repair_type,
                        repair_type_name:this.form.repair_type_name
                    };
                    request(url,method,params).then((res,err)=>{
                    if(res){
      ...
      handleEditItem(){
          ...
          if(this.multipleSelection!=null&&this.multipleSelection!=""
                &&this.multipleSelection.length==1){
                    this.form.repair_type_name = this.multipleSelection[0].repair_type_name;
                    this.dialogFormVisible = true;
          }else{
              Message({
                    message:'请选择一项后再编辑!',
                ...
                this.$confirm('确认删除？')
                    .then(()=> {
                        ...
                        for(var i=0;i<this.multipleSelection.length;i++){
                            delete_Array.push(this.deleteItemAsync(this.multipleSelection[i].repair_type));
                        }
      ...
      //查询
      handleSearch(){
         this.getDataPageNation("",this.repair_type_name,this.CurrentPage,this.CurrentPageSize);
      },
      //删除
      deleteItemAsync(repair_type){
        var url = UrlConfig.DeleteRepairType;
        var method ='post';
        var params =  {
      ...
      //分页获取数据
      getDataPageNation(repair_type, repair_type_name, pageIndex, pageSize){
